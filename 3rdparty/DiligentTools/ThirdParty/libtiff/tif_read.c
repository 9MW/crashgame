...
static tmsize_t
TIFFReadRawStrip1(TIFF* tif, uint32 strip, void* buf, tmsize_t size,const char* tfmodule);
static tmsize_t
TIFFReadRawTile1(TIFF* tif, uint32 tile, void* buf, tmsize_t size, const char* tfmodule);
...
{
	static const char tfmodule[] = "TIFFFillStripPartial";
...
{
	static const char tfmodule[] = "TIFFReadEncodedStrip";
...
TIFFReadEncodedStrip(TIFF* tif, uint32 strip, void* buf, tmsize_t size)
{
    ...
    {
        if (TIFFReadRawStrip1(tif, strip, buf, stripsize, tfmodule) != stripsize)
            return ((tmsize_t)(-1));
...
TIFFReadRawStripOrTile2(TIFF* tif, uint32 strip_or_tile, int is_strip,
                        tmsize_t size, const char* tfmodule)
{
        assert( !isMapped(tif) );
        assert((tif->tif_flags&TIFF_NOREADRAW)==0);
...
{
	static const char tfmodule[] = "TIFFReadRawStrip";
	TIFFDirectory *td = &tif->tif_dir;
	uint64 bytecount;
	tmsize_t bytecountm;
	...
	if (size != (tmsize_t)(-1) && size < bytecountm)
		bytecountm = size;
	return (TIFFReadRawStrip1(tif, strip, buf, bytecountm, tfmodule));
}
...
{
	static const char tfmodule[] = "TIFFFillStrip";
	TIFFDirectory *td = &tif->tif_dir;
		...
		if (isMapped(tif) &&
			...
			if( isMapped(tif) )
			{
				...
				if (TIFFReadRawStrip1(tif, strip, tif->tif_rawdata,
				    bytecountm, tfmodule) != bytecountm)
				{
					return (0);
				}
			}
...
{
	static const char tfmodule[] = "TIFFReadEncodedTile";
	TIFFDirectory *td = &tif->tif_dir;
    ...
    if( td->td_compression == COMPRESSION_NONE &&
        ...
        ((tif->tif_flags&TIFF_NOREADRAW)==0) )
    {
        if (TIFFReadRawTile1(tif, tile, buf, tilesize, tfmodule) != tilesize)
            return ((tmsize_t)(-1));
...
_TIFFReadEncodedTileAndAllocBuffer(TIFF* tif, uint32 tile,
                                    ...
                                    tmsize_t size_to_read)
{
    static const char tfmodule[] = "_TIFFReadEncodedTileAndAllocBuffer";
    TIFFDirectory *td = &tif->tif_dir;
    tmsize_t tilesize = tif->tif_tilesize;
...
{
	static const char tfmodule[] = "TIFFReadRawTile";
	TIFFDirectory *td = &tif->tif_dir;
	uint64 bytecount64;
	tmsize_t bytecountm;
	...
	return (TIFFReadRawTile1(tif, tile, buf, bytecountm, tfmodule));
}
...
{
	static const char tfmodule[] = "TIFFFillTile";
	TIFFDirectory *td = &tif->tif_dir;
...
{
	static const char tfmodule[] = "TIFFReadBufferSetup";
...
{
        static const char tfmodule[] = "TIFFStartTile";
	TIFFDirectory *td = &tif->tif_dir;
        uint32 howmany32;
...
{
    static const char tfmodule[] = "TIFFReadFromUserBuffer";
    TIFFDirectory *td = &tif->tif_dir;
    int ret = 1;
