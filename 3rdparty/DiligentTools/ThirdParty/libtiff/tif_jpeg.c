...
JPEGFixupTagsSubsampling(TIFF* tif)
{
	 ...
	 */
	static const char tfmodule[] = "JPEGFixupTagsSubsampling";
...
{
	static const char tfmodule[] = "JPEGFixupTagsSubsamplingSec";
	uint8 m;
	while (1)
	{
		while (1)
			...
			case JPEG_MARKER_SOF10: /* Progressive arithmetic: normally not allowed by TechNote, but that doesn't hurt supporting it */
				/* this marker contains the subsampling factors we're scanning for */
				{
					...
					for (o=1; o<data->tif->tif_dir.td_samplesperpixel; o++)
					{
						...
						if (p!=0x11)
						{
							TIFFWarningExt(data->tif->tif_clientdata,tfmodule,
...
JPEGPreDecode(TIFF* tif, uint16 s)
{
	JPEGState *sp = JState(tif);
	TIFFDirectory *td = &tif->tif_dir;
	static const char tfmodule[] = "JPEGPreDecode";
	...
	if (sp->cinfo.d.num_components !=
		...
		TIFFErrorExt(tif->tif_clientdata, tfmodule, "Improper JPEG component count");
		return (0);
	}
#ifdef JPEG_LIB_MK1
	if (12 != td->td_bitspersample && 8 != td->td_bitspersample) {
		TIFFErrorExt(tif->tif_clientdata, tfmodule, "Improper JPEG data precision");
		return (0);
	}
	...
	if (td->td_planarconfig == PLANARCONFIG_CONTIG) {
		...
		for (ci = 1; ci < sp->cinfo.d.num_components; ci++) {
			if (sp->cinfo.d.comp_info[ci].h_samp_factor != 1 ||
				...
				TIFFErrorExt(tif->tif_clientdata, tfmodule, "Improper JPEG sampling factors");
				return (0);
			}
		}
	} else {
...
JPEGSetupEncode(TIFF* tif)
{
	JPEGState* sp = JState(tif);
	TIFFDirectory *td = &tif->tif_dir;
	static const char tfmodule[] = "JPEGSetupEncode";
...
#endif
	{
		TIFFErrorExt(tif->tif_clientdata, tfmodule, "BitsPerSample %d not allowed for JPEG",
...
JPEGPreEncode(TIFF* tif, uint16 s)
{
	JPEGState *sp = JState(tif);
	TIFFDirectory *td = &tif->tif_dir;
	static const char tfmodule[] = "JPEGPreEncode";
	uint32 segment_width, segment_height;
	...
	if (segment_width > 65535 || segment_height > 65535) {
		TIFFErrorExt(tif->tif_clientdata, tfmodule, "Strip/tile too large for JPEG");
		return (0);
	}
