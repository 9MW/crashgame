...
{
	static const char tfmodule[]="TIFFInitOJPEG";
	OJPEGState* sp;
	...
	if (!_TIFFMergeFields(tif, ojpegFields, TIFFArrayCount(ojpegFields))) {
		TIFFErrorExt(tif->tif_clientdata, tfmodule,
	...
	if (sp==NULL)
	{
		TIFFErrorExt(tif->tif_clientdata,tfmodule,"No space for OJPEG state block");
		return(0);
	}
...
{
	static const char tfmodule[]="OJPEGVSetField";
	OJPEGState* sp=(OJPEGState*)tif->tif_data;
	uint32 ma;
	uint64* mb;
	uint32 n;
	...
	switch(tag)
	{
		...
		case TIFFTAG_JPEGQTABLES:
			...
			if (ma!=0)
			{
				if (ma>3)
				{
					TIFFErrorExt(tif->tif_clientdata,tfmodule,"JpegQTables tag has incorrect count");
					return(0);
				}
		...
		case TIFFTAG_JPEGDCTABLES:
			...
			if (ma!=0)
			{
				if (ma>3)
				{
					TIFFErrorExt(tif->tif_clientdata,tfmodule,"JpegDcTables tag has incorrect count");
					return(0);
				}
		...
		case TIFFTAG_JPEGACTABLES:
			...
			if (ma!=0)
			{
				if (ma>3)
				{
					TIFFErrorExt(tif->tif_clientdata,tfmodule,"JpegAcTables tag has incorrect count");
					return(0);
				}
...
{
	static const char tfmodule[]="OJPEGSetupDecode";
	TIFFWarningExt(tif->tif_clientdata,tfmodule,"Depreciated and troublesome old-style JPEG compression mode, please convert to new-style JPEG compression and notify vendor of writing software");
	return(1);
}
...
{
	static const char tfmodule[]="OJPEGPreDecodeSkipScanlines";
	OJPEGState* sp=(OJPEGState*)tif->tif_data;
	uint32 m;
	if (sp->skip_buffer==NULL)
	{
		...
		if (sp->skip_buffer==NULL)
		{
			TIFFErrorExt(tif->tif_clientdata,tfmodule,"Out of memory");
			return(0);
		}
	}
...
{
        static const char tfmodule[]="OJPEGDecode";
	OJPEGState* sp=(OJPEGState*)tif->tif_data;
	(void)s;
        if( !sp->decoder_ok )
        {
            TIFFErrorExt(tif->tif_clientdata,tfmodule,"Cannot decode: decoder not correctly initialized");
            return 0;
        }
	if (sp->libjpeg_jpeg_query_style==0)
	{
...
{
	static const char tfmodule[]="OJPEGDecodeRaw";
	OJPEGState* sp=(OJPEGState*)tif->tif_data;
	uint8* m;
	tmsize_t n;
	uint8* oy;
	...
	{
		TIFFErrorExt(tif->tif_clientdata,tfmodule,"Fractional scanline not read");
		return(0);
	}
	assert(cc>0);
	m=buf;
...
{
	static const char tfmodule[]="OJPEGDecodeScanlines";
	OJPEGState* sp=(OJPEGState*)tif->tif_data;
	uint8* m;
	tmsize_t n;
	if (cc%sp->bytes_per_line!=0)
...
{
	static const char tfmodule[]="OJPEGSetupEncode";
	TIFFErrorExt(tif->tif_clientdata,tfmodule,"OJPEG encoding not supported; use new-style JPEG compression instead");
	return(0);
}
...
{
	static const char tfmodule[]="OJPEGPreEncode";
	(void)s;
	TIFFErrorExt(tif->tif_clientdata,tfmodule,"OJPEG encoding not supported; use new-style JPEG compression instead");
	return(0);
}
...
{
	static const char tfmodule[]="OJPEGEncode";
	(void)buf;
	(void)cc;
	(void)s;
	TIFFErrorExt(tif->tif_clientdata,tfmodule,"OJPEG encoding not supported; use new-style JPEG compression instead");
...
{
	static const char tfmodule[]="OJPEGPostEncode";
	TIFFErrorExt(tif->tif_clientdata,tfmodule,"OJPEG encoding not supported; use new-style JPEG compression instead");
	return(0);
}
...
{
	static const char tfmodule[]="OJPEGSubsamplingCorrect";
	OJPEGState* sp=(OJPEGState*)tif->tif_data;
	uint8 mh;
	uint8 mv;
	    ...
	    (tif->tif_dir.td_photometric!=PHOTOMETRIC_ITULAB)))
	{
		if (sp->subsampling_tag!=0)
			TIFFWarningExt(tif->tif_clientdata,tfmodule,"Subsampling tag not appropriate for this Photometric and/or SamplesPerPixel");
	...
	else
	{
		...
		{
			if (sp->subsampling_tag==0)
				TIFFWarningExt(tif->tif_clientdata,tfmodule,"Subsampling tag is not set, yet subsampling inside JPEG data [%d,%d] does not match default values [2,2]; assuming subsampling inside JPEG data is correct",sp->subsampling_hor,sp->subsampling_ver);
			else
				TIFFWarningExt(tif->tif_clientdata,tfmodule,"Subsampling inside JPEG data [%d,%d] does not match subsampling tag values [%d,%d]; assuming subsampling inside JPEG data is correct",sp->subsampling_hor,sp->subsampling_ver,mh,mv);
		}
		if (sp->subsampling_force_desubsampling_inside_decompression!=0)
		{
			if (sp->subsampling_tag==0)
				TIFFWarningExt(tif->tif_clientdata,tfmodule,"Subsampling tag is not set, yet subsampling inside JPEG data does not match default values [2,2] (nor any other values allowed in TIFF); assuming subsampling inside JPEG data is correct and desubsampling inside JPEG decompression");
			else
				TIFFWarningExt(tif->tif_clientdata,tfmodule,"Subsampling inside JPEG data does not match subsampling tag values [%d,%d] (nor any other values allowed in TIFF); assuming subsampling inside JPEG data is correct and desubsampling inside JPEG decompression",mh,mv);
		}
		if (sp->subsampling_force_desubsampling_inside_decompression==0)
		{
			if (sp->subsampling_hor<sp->subsampling_ver)
				TIFFWarningExt(tif->tif_clientdata,tfmodule,"Subsampling values [%d,%d] are not allowed in TIFF",sp->subsampling_hor,sp->subsampling_ver);
		}
	}
	sp->subsamplingcorrect_done=1;
}
...
{
	static const char tfmodule[]="OJPEGReadHeaderInfo";
	OJPEGState* sp=(OJPEGState*)tif->tif_data;
	...
	else
	{
		...
		{
			TIFFErrorExt(tif->tif_clientdata,tfmodule,"SamplesPerPixel %d not supported for this compression scheme",sp->samples_per_pixel);
			return(0);
		}
	...
	if (sp->strile_length<sp->image_length)
	{
		...
		{
			TIFFErrorExt(tif->tif_clientdata,tfmodule,"Incompatible vertical subsampling and image strip/tile length");
			return(0);
		}
...
{
	static const char tfmodule[]="OJPEGWriteHeaderInfo";
	OJPEGState* sp=(OJPEGState*)tif->tif_data;
	uint8** m;
	uint32 n;
...
{
	static const char tfmodule[]="OJPEGReadHeaderInfoSec";
	OJPEGState* sp=(OJPEGState*)tif->tif_data;
	uint8 m;
	uint16 n;
	uint8 o;
	...
	do
	{
		...
		switch(m)
		{
			...
			case JPEG_MARKER_APP0+15:
				...
				if (n<2)
				{
					if (sp->subsamplingcorrect==0)
						TIFFErrorExt(tif->tif_clientdata,tfmodule,"Corrupt JPEG data");
					return(0);
				}
				if (n>2)
			...
			case JPEG_MARKER_SOS:
				...
				break;
			default:
				TIFFErrorExt(tif->tif_clientdata,tfmodule,"Unknown marker type %d in JPEG data",m);
				return(0);
		}
...
OJPEGReadHeaderInfoSecStreamDri(TIFF* tif)
{
	/* This could easily cause trouble in some cases... but no such cases have
           occurred so far */
	static const char tfmodule[]="OJPEGReadHeaderInfoSecStreamDri";
	OJPEGState* sp=(OJPEGState*)tif->tif_data;
	uint16 m;
	...
	if (m!=4)
	{
		TIFFErrorExt(tif->tif_clientdata,tfmodule,"Corrupt DRI marker in JPEG data");
		return(0);
	}
	if (OJPEGReadWord(sp,&m)==0)
		return(0);
...
OJPEGReadHeaderInfoSecStreamDqt(TIFF* tif)
{
	...
	static const char tfmodule[]="OJPEGReadHeaderInfoSecStreamDqt";
	OJPEGState* sp=(OJPEGState*)tif->tif_data;
	uint16 m;
	uint32 na;
	uint8* nb;
	...
	{
		if (sp->subsamplingcorrect==0)
			TIFFErrorExt(tif->tif_clientdata,tfmodule,"Corrupt DQT marker in JPEG data");
		return(0);
	}
	if (sp->subsamplingcorrect!=0)
...
OJPEGReadHeaderInfoSecStreamDht(TIFF* tif)
{
	...
	static const char tfmodule[]="OJPEGReadHeaderInfoSecStreamDht";
	OJPEGState* sp=(OJPEGState*)tif->tif_data;
	uint16 m;
	uint32 na;
	uint8* nb;
	...
	{
		if (sp->subsamplingcorrect==0)
			TIFFErrorExt(tif->tif_clientdata,tfmodule,"Corrupt DHT marker in JPEG data");
		return(0);
	}
	if (sp->subsamplingcorrect!=0)
	{
...
OJPEGReadHeaderInfoSecStreamSof(TIFF* tif, uint8 marker_id)
{
	...
	static const char tfmodule[]="OJPEGReadHeaderInfoSecStreamSof";
	OJPEGState* sp=(OJPEGState*)tif->tif_data;
	uint16 m;
	uint16 n;
	uint8 o;
	...
	{
		if (sp->subsamplingcorrect==0)
			TIFFErrorExt(tif->tif_clientdata,tfmodule,"Corrupt SOF marker in JPEG data");
		return(0);
	}
	m-=8;
	if (m%3!=0)
	...
	if (sp->subsamplingcorrect==0)
	{
		...
		{
			TIFFErrorExt(tif->tif_clientdata,tfmodule,"JPEG compressed data indicates unexpected number of samples");
			return(0);
		}
	}
	/* P: Sample precision */
	...
	{
		if (sp->subsamplingcorrect==0)
			TIFFErrorExt(tif->tif_clientdata,tfmodule,"JPEG compressed data indicates unexpected number of bits per sample");
		return(0);
	}
	...
	else
	{
		...
		{
			TIFFErrorExt(tif->tif_clientdata,tfmodule,"JPEG compressed data indicates unexpected height");
			return(0);
		}
		sp->sof_y=p;
		...
		{
			TIFFErrorExt(tif->tif_clientdata,tfmodule,"JPEG compressed data indicates unexpected width");
			return(0);
		}
		if ((uint32)p>sp->strile_width)
		{
			TIFFErrorExt(tif->tif_clientdata,tfmodule,"JPEG compressed data image width exceeds expected image width");
			return(0);
		}
		sp->sof_x=p;
	}
		...
		else
		{
			...
			if (sp->subsampling_force_desubsampling_inside_decompression==0)
			{
				if (q==0)
				{
					...
					{
						TIFFErrorExt(tif->tif_clientdata,tfmodule,"JPEG compressed data indicates unexpected subsampling values");
						return(0);
					}
				}
				else
...
{
	/* this marker needs to be checked, and part of its data needs to be saved for regeneration later on */
	static const char tfmodule[]="OJPEGReadHeaderInfoSecStreamSos";
	OJPEGState* sp=(OJPEGState*)tif->tif_data;
	uint16 m;
	uint8 n;
	uint8 o;
	...
	if (sp->sof_log==0)
	{
		TIFFErrorExt(tif->tif_clientdata,tfmodule,"Corrupt SOS marker in JPEG data");
		return(0);
	}
	/* Ls */
	if (OJPEGReadWord(sp,&m)==0)
...
{
	static const char tfmodule[]="OJPEGReadHeaderInfoSecTablesQTable";
	OJPEGState* sp=(OJPEGState*)tif->tif_data;
	uint8 m;
	uint8 n;
	uint32 oa;
	...
	{
		TIFFErrorExt(tif->tif_clientdata,tfmodule,"Missing JPEG tables");
		return(0);
	}
	sp->in_buffer_file_pos_log=0;
	for (m=0; m<sp->samples_per_pixel; m++)
	{
		if ((sp->qtable_offset[m]!=0) && ((m==0) || (sp->qtable_offset[m]!=sp->qtable_offset[m-1])))
		{
			for (n=0; n<m-1; n++)
			{
				...
				{
					TIFFErrorExt(tif->tif_clientdata,tfmodule,"Corrupt JpegQTables tag value");
					return(0);
				}
			}
			oa=sizeof(uint32)+69;
...
{
	static const char tfmodule[]="OJPEGReadHeaderInfoSecTablesDcTable";
	OJPEGState* sp=(OJPEGState*)tif->tif_data;
	uint8 m;
	uint8 n;
	uint8 o[16];
	...
	for (m=0; m<sp->samples_per_pixel; m++)
	{
		if ((sp->dctable_offset[m]!=0) && ((m==0) || (sp->dctable_offset[m]!=sp->dctable_offset[m-1])))
		{
			for (n=0; n<m-1; n++)
			{
				...
				{
					TIFFErrorExt(tif->tif_clientdata,tfmodule,"Corrupt JpegDcTables tag value");
					return(0);
				}
			}
...
{
	static const char tfmodule[]="OJPEGReadHeaderInfoSecTablesAcTable";
	OJPEGState* sp=(OJPEGState*)tif->tif_data;
	uint8 m;
	uint8 n;
	uint8 o[16];
	...
	for (m=0; m<sp->samples_per_pixel; m++)
	{
		if ((sp->actable_offset[m]!=0) && ((m==0) || (sp->actable_offset[m]!=sp->actable_offset[m-1])))
		{
			for (n=0; n<m-1; n++)
			{
				...
				{
					TIFFErrorExt(tif->tif_clientdata,tfmodule,"Corrupt JpegAcTables tag value");
					return(0);
				}
			}
