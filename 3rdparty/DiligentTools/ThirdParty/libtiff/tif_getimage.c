...
{
	static const char tfmodule[] = "initYCbCrConversion";
	...
	if (img->ycbcr == NULL) {
		...
		if (img->ycbcr == NULL) {
			TIFFErrorExt(img->tif->tif_clientdata, tfmodule,
...
{
	static const char tfmodule[] = "initCIELabConversion";
...
{
	static const char tfmodule[]="BuildMapUaToAa";
	uint8* m;
	uint16 na,nv;
	...
	if (img->UaToAa==NULL)
	{
		TIFFErrorExt(img->tif->tif_clientdata,tfmodule,"Out of memory");
		return(0);
	}
	m=img->UaToAa;
	for (na=0; na<256; na++)
...
{
	static const char tfmodule[]="BuildMapBitdepth16To8";
	uint8* m;
	uint32 n;
