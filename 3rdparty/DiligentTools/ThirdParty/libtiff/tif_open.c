...
int
_TIFFgetMode(const char* mode, const char* tfmodule)
{
	int m = -1;
	...
	case 'a':
		...
		break;
	default:
		TIFFErrorExt(0, tfmodule, "\"%s\": Bad mode", mode);
		break;
	}
	return (m);
}
...
{
	static const char tfmodule[] = "TIFFClientOpen";
	TIFF *tif;
	int m;
	const char* cp;
	...
	if (tif == NULL) {
		TIFFErrorExt(clientdata, tfmodule, "%s: Out of memory (TIFF structure)", name);
		goto bad2;
	}
	...
	if (!readproc || !writeproc || !seekproc || !closeproc || !sizeproc) {
		TIFFErrorExt(clientdata, tfmodule,
