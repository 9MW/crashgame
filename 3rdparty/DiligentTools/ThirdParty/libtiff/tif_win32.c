...
TIFFFdOpen(int ifd, const char* name, const char* mode)
{
	...
	tif = TIFFClientOpen(name, mode, (thandle_t)(size_t)ifd, /* FIXME: WIN64 cast to pointer warning */
...
{
	static const char tfmodule[] = "TIFFOpen";
	thandle_t fd;
	int m;
	DWORD dwMode;
	TIFF* tif;

	m = _TIFFgetMode(mode, tfmodule);
	...
	if (fd == INVALID_HANDLE_VALUE) {
		TIFFErrorExt(0, tfmodule, "%s: Cannot open", name);
		return ((TIFF *)0);
	}

	tif = TIFFFdOpen((int)(size_t)fd, name, mode);   /* FIXME: WIN64 cast from pointer to int warning */
	if(!tif)
		CloseHandle(fd);
	return tif;
}
...
{
	static const char tfmodule[] = "TIFFOpenW";
	thandle_t fd;
	int m;
	DWORD dwMode;
	int mbsize;
	...
	tif = TIFFFdOpen((int)(size_t)fd,    /* FIXME: WIN64 cast from pointer to int warning */
...
static void
Win32WarningHandler(const char* tfmodule, const char* fmt, va_list ap)
{
	if (tfmodule != NULL)
		fprintf(stderr, "%s: ", tfmodule);
...
static void
Win32ErrorHandler(const char* tfmodule, const char* fmt, va_list ap)
{
	if (tfmodule != NULL)
		fprintf(stderr, "%s: ", tfmodule);
	vfprintf(stderr, fmt, ap);
