...
{
	static const char tfmodule[] = "PixarLogSetupDecode";
	...
	if (inflateInit(&sp->stream) != Z_OK) {
                ...
                sp->tbuf = NULL;
                sp->tbuf_size = 0;
		TIFFErrorExt(tif->tif_clientdata, tfmodule, "%s", sp->stream.msg ? sp->stream.msg : "(null)");
		return (0);
	} else {
...
{
	static const char tfmodule[] = "PixarLogPreDecode";
	PixarLogState* sp = DecoderState(tif);
	...
	{
		TIFFErrorExt(tif->tif_clientdata, tfmodule, "ZLib cannot deal with buffers this size");
		return (0);
	}
...
{
	static const char tfmodule[] = "PixarLogDecode";
	TIFFDirectory *td = &tif->tif_dir;
	PixarLogState* sp = DecoderState(tif);
	tmsize_t i;
	tmsize_t nsamples;
	...
	{
		TIFFErrorExt(tif->tif_clientdata, tfmodule, "sp->stream.avail_out > sp->tbuf_size");
		return (0);
	}
	do {
		...
		if (state != Z_OK) {
			TIFFErrorExt(tif->tif_clientdata, tfmodule, "ZLib error: %s",
...
{
	static const char tfmodule[] = "PixarLogSetupEncode";
	TIFFDirectory *td = &tif->tif_dir;
	...
	if (sp->user_datafmt == PIXARLOGDATAFMT_UNKNOWN)
		sp->user_datafmt = PixarLogGuessDataFmt(td);
	if (sp->user_datafmt == PIXARLOGDATAFMT_UNKNOWN) {
		TIFFErrorExt(tif->tif_clientdata, tfmodule, "PixarLog compression can't handle %d bit linear encodings", td->td_bitspersample);
		return (0);
	}
...
{
	static const char tfmodule[] = "PixarLogPreEncode";
	PixarLogState *sp = EncoderState(tif);
...
{
	static const char tfmodule[] = "PixarLogEncode";
	TIFFDirectory *td = &tif->tif_dir;
	PixarLogState *sp = EncoderState(tif);
	tmsize_t i;
	tmsize_t n;
	...
	do {
		if (deflate(&sp->stream, Z_NO_FLUSH) != Z_OK) {
			TIFFErrorExt(tif->tif_clientdata, tfmodule, "Encoder error: %s",
			    sp->stream.msg ? sp->stream.msg : "(null)");
			return (0);
		}
		if (sp->stream.avail_out == 0) {
...
{
	static const char tfmodule[] = "PixarLogPostEncode";
	PixarLogState *sp = EncoderState(tif);
	int state;
...
{
    static const char tfmodule[] = "PixarLogVSetField";
    PixarLogState *sp = (PixarLogState *)tif->tif_data;
    int result;
...
{
	static const char tfmodule[] = "TIFFInitPixarLog";
