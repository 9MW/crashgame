...
#define EXTRASAMPLE_COREL_UNASSALPHA 999 
	...
	uint32 i;
        ...
        static const char tfmodule[] = "setExtraSamples";
...
{
	static const char tfmodule[] = "_TIFFVSetField";
...
{
	static const char tfmodule[] = "TIFFAdvanceDirectory";
	if (isMapped(tif))
	{
		uint64 poff=*nextdir;
		if (!(tif->tif_flags&TIFF_BIGTIFF))
		{
			...
			{
				TIFFErrorExt(tif->tif_clientdata,tfmodule,"Error fetching directory count");
                                  *nextdir=0;
				return(0);
			}
			...
			{
				TIFFErrorExt(tif->tif_clientdata,tfmodule,"Error fetching directory link");
				return(0);
			}
			if (off!=NULL)
				*off=(uint64)poffc;
		...
		else
		{
			...
			{
				TIFFErrorExt(tif->tif_clientdata,tfmodule,"Sanity check on directory count failed");
				return(0);
			}
	...
	else
	{
		if (!(tif->tif_flags&TIFF_BIGTIFF))
		{
			...
			if (!SeekOK(tif, *nextdir) ||
				...
				TIFFErrorExt(tif->tif_clientdata, tfmodule, "%s: Error fetching directory count",
				    tif->tif_name);
				return (0);
			}
			if (tif->tif_flags & TIFF_SWAB)
			...
			if (!ReadOK(tif, &nextdir32, sizeof (uint32))) {
				TIFFErrorExt(tif->tif_clientdata, tfmodule, "%s: Error fetching directory link",
				    tif->tif_name);
				return (0);
			}
			if (tif->tif_flags & TIFF_SWAB)
...
{
	static const char tfmodule[] = "TIFFNumberOfDirectories";
	uint64 nextdir;
	uint16 n;
	if (!(tif->tif_flags&TIFF_BIGTIFF))
...
{
	static const char tfmodule[] = "TIFFUnlinkDirectory";
	uint64 nextdir;
	uint64 off;
	uint16 n;
	...
	for (n = dirn-1; n > 0; n--) {
		if (nextdir == 0) {
			TIFFErrorExt(tif->tif_clientdata, tfmodule, "Directory %d does not exist", dirn);
			return (0);
		}
	...
	if (!(tif->tif_flags&TIFF_BIGTIFF))
	{
		...
		if (!WriteOK(tif, &nextdir32, sizeof (uint32))) {
			TIFFErrorExt(tif->tif_clientdata, tfmodule, "Error writing directory link");
			return (0);
		}
	}
	else
