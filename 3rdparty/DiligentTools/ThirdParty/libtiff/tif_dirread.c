...
static void TIFFReadDirEntryOutputErr(TIFF* tif, enum TIFFReadDirEntryErr err, const char* tfmodule, const char* tagname, int recover);
...
static void TIFFReadDirEntryOutputErr(TIFF* tif, enum TIFFReadDirEntryErr err, const char* tfmodule, const char* tagname, int recover)
{
	if (!recover) {
		...
		switch (err) {
			case TIFFReadDirEntryErrCount:
				TIFFWarningExt(tif->tif_clientdata, tfmodule,
...
{
	static const char tfmodule[] = "TIFFReadDirectory";
	TIFFDirEntry* dir;
	uint16 dircount;
	TIFFDirEntry* dp;
	uint16 di;
	...
	if (dp)
	{
		...
		if (err!=TIFFReadDirEntryErrOk)
		{
			TIFFReadDirEntryOutputErr(tif,err,tfmodule,"Compression",0);
			goto bad;
		}
			...
			case TIFFTAG_SAMPLEFORMAT:
				 ...
				 */
				{
					...
					if (err!=TIFFReadDirEntryErrOk)
					{
						...
						TIFFReadDirEntryOutputErr(tif,err,tfmodule,fip ? fip->field_name : "unknown tagname",0);
						goto bad;
					}
			...
			case TIFFTAG_TRANSFERFUNCTION:
				{
                    ...
                    {
						fip = TIFFFieldWithTag(tif,dp->tdir_tag);
						TIFFReadDirEntryOutputErr(tif,err,tfmodule,fip ? fip->field_name : "unknown tagname",1);
                    }
					else
					{
...
/* END REV 4.0 COMPATIBILITY */
	 ...
	 */
        ...
        if (color_channels && tif->tif_dir.td_samplesperpixel - tif->tif_dir.td_extrasamples > color_channels) {
                ...
                TIFFWarningExt(tif->tif_clientdata,tfmodule, "Sum of Photometric type-related "
                ...
                if (!new_sampleinfo) {
                    TIFFErrorExt(tif->tif_clientdata, tfmodule, "Failed to allocate memory for "
...
{
	static const char tfmodule[] = "TIFFReadDirectoryCheckOrder";
	uint16 m;
	uint16 n;
	TIFFDirEntry* o;
	m=0;
...
{
	static const char tfmodule[] = "TIFFReadCustomDirectory";
	TIFFDirEntry* dir;
	uint16 dircount;
	TIFFDirEntry* dp;
	uint16 di;
...
{
	static const char tfmodule[] = "EstimateStripByteCounts";
...
{
	static const char tfmodule[] = "MissingRequired";
...
TIFFFetchDirectory(TIFF* tif, uint64 diroff, TIFFDirEntry** pdir,
                   uint64 *nextdiroff)
{
	static const char tfmodule[] = "TIFFFetchDirectory";
	...
	if (!isMapped(tif)) {
		...
		{
			TIFFErrorExt(tif->tif_clientdata,tfmodule,"Can not read TIFF directory count");
			return(0);
		}
...
{
	static const char tfmodule[] = "TIFFFetchNormalTag";
	enum TIFFReadDirEntryErr err;
	uint32 fii;
		...
		case TIFF_SETGET_ASCII:
			{
				...
				if (err==TIFFReadDirEntryErrOk)
				{
					...
					if (mb+1<(uint32)dp->tdir_count)
						TIFFWarningExt(tif->tif_clientdata,tfmodule,"ASCII value for tag \"%s\" contains null byte in value; value incorrectly truncated during reading due to implementation limitations",fip->field_name);
					else if (mb+1>(uint32)dp->tdir_count)
					{
						uint8* o;
						TIFFWarningExt(tif->tif_clientdata,tfmodule,"ASCII value for tag \"%s\" does not end in null byte",fip->field_name);
		...
		case TIFF_SETGET_C16_ASCII:
			{
				...
				else
				{
					...
					if (err==TIFFReadDirEntryErrOk)
					{
						...
						{
						    TIFFWarningExt(tif->tif_clientdata,tfmodule,"ASCII value for tag \"%s\" does not end in null byte. Forcing it to be null",fip->field_name);
	...
	if (err!=TIFFReadDirEntryErrOk)
	{
		TIFFReadDirEntryOutputErr(tif,err,tfmodule,fip->field_name,recover);
		return(0);
	}
	return(1);
}
...
{
	static const char tfmodule[] = "TIFFFetchStripThing";
	enum TIFFReadDirEntryErr err;
	uint64* data;
...
{
	static const char tfmodule[] = "TIFFFetchSubjectDistance";
	enum TIFFReadDirEntryErr err;
	UInt64Aligned_t m;
    m.l=0;
	assert(sizeof(double)==8);
	...
	{
		TIFFReadDirEntryOutputErr(tif,err,tfmodule,"SubjectDistance",TRUE);
		return(0);
	}
}
...
{
    static const char tfmodule[] = "_TIFFPartialReadStripArray";
...
static int _TIFFFetchStrileValue(TIFF* tif,
                                 ...
                                 uint64** parray)
{
    static const char tfmodule[] = "_TIFFFetchStrileValue";
    TIFFDirectory *td = &tif->tif_dir;
    ...
    if( strile >= td->td_stripoffsetbyteallocsize )
    {
        ...
        if( strile > 1000000 )
        {
            ...
            {
                TIFFErrorExt(tif->tif_clientdata, tfmodule, "File too short");
                return 0;
            }
        }
