...
namespace nonstd {
    namespace span_lite {
        ...
        span_constexpr const extent_t dynamic_extent_SL = static_cast<extent_t>(-1);

        template< class T, extent_t Extent = dynamic_extent_SL >
        class span;
...
#define span_sizeof(T)  static_cast<extent_t>( sizeof(T) )
        ...
        public:
            ...
            template< std::size_t N
                span_REQUIRES_T((
                    (Extent == dynamic_extent_SL || Extent == static_cast<extent_t>(N))
...
# endif
            ...
            template< class OtherElementType, extent_type OtherExtent
                span_REQUIRES_T((
                    (Extent == dynamic_extent_SL || Extent == OtherExtent)
            ...
            >
                ...
                , size_(other.size())
            {
                span_EXPECTS(OtherExtent == dynamic_extent_SL || other.size() == to_size(OtherExtent));
            }
...
#if span_HAVE( DEFAULT_FUNCTION_TEMPLATE_ARG )
            template< size_type Offset, extent_type Count = dynamic_extent_SL >
#else
            template< size_type Offset, extent_type Count /*= dynamic_extent_SL*/ >
#endif
            span_constexpr_exp span< element_type, Count >
                subspan() const
            {
                span_EXPECTS(
                    ...
                    (Count == dynamic_extent_SL || (detail::is_positive(Count) && Count + Offset <= size()))
                );

                return span< element_type, Count >(
                    data() + Offset, Count != dynamic_extent_SL ? Count : (Extent != dynamic_extent_SL ? Extent - Offset : size() - Offset));
            }

            span_constexpr_exp span< element_type, dynamic_extent_SL >
                first(size_type count) const
            {
                span_EXPECTS(detail::is_positive(count) && count <= size());

                return span< element_type, dynamic_extent_SL >(data(), count);
            }
                ...
                last(size_type count) const
            {
                ...
                return span< element_type, dynamic_extent_SL >(data() + (size() - count), count);
            }

            span_constexpr_exp span< element_type, dynamic_extent_SL >
                subspan(size_type offset, size_type count = static_cast<size_type>(dynamic_extent_SL)) const
            {
                span_EXPECTS(
                    ((detail::is_positive(offset) && offset <= size())) &&
                    (count == static_cast<size_type>(dynamic_extent_SL) || (detail::is_positive(count) && offset + count <= size()))
                );

                return span< element_type, dynamic_extent_SL >(
                    data() + offset, count == static_cast<size_type>(dynamic_extent_SL) ? size() - offset : count);
            }
...
// Avoid MSVC 14.1 (1910), VS 2017: warning C4307: '*': integral constant overflow:
        ...
        template< typename T >
        struct BytesExtent< T, dynamic_extent_SL >
        {
#if span_CPP11_OR_GREATER
            enum ET : extent_t {
                value = dynamic_extent_SL
            };
#else
            enum ET {
                value = dynamic_extent_SL
...
namespace nonstd {
    ...
    using span_lite::dynamic_extent_SL;
...
namespace nonstd {
    namespace span_lite {
        ...
        template< class T, extent_t Extent >
        span_constexpr span<T>
            subspan(span<T, Extent> spn, size_t offset, extent_t count = dynamic_extent_SL)
        {
        ...
        template< size_t Offset, extent_t Count = dynamic_extent_SL, class T >
        span_constexpr auto
        ...
        template< class T >
        span_constexpr auto
            subspan(T& t, size_t offset, extent_t count = dynamic_extent_SL) -> decltype(make_span(t).subspan(offset, count))
        {
...
namespace std {
    ...
    class tuple_size< nonstd::span<ElementType, nonstd::dynamic_extent_SL> >;
    ...
    public:
        ...
        static_assert(Extent != nonstd::dynamic_extent_SL && I < Extent, "tuple_element<I,span>: dynamic extent or index out of range");
#endif
    ...
    {
#if span_HAVE( STATIC_ASSERT )
        static_assert(Extent != nonstd::dynamic_extent_SL && I < Extent, "get<>(span): dynamic extent or index out of range");
#endif
        return spn[I];
    }
