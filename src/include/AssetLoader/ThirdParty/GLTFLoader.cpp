                     ...
                     std::vector<VertexAttribs1>& vertexData1)
{
    ...
    if (gltf_node.mesh > -1)
    {
        ...
        for (size_t j = 0; j < gltf_mesh.primitives.size(); j++)
        {
            ...
            //        indexCount,
            //        vertexCount,
            //        primitive.material > -1 ? primitive.material : (int)Materials.size()-1 //
            //    }                                                                              //
            //);
            ...
            NewMesh->Primitives.emplace_back(
                ...
                indexCount,
                vertexCount,
                primitive.material > -1 ? primitive.material : Materials.size() - 1
            );
            NewMesh->Primitives.back().SetBoundingBox(PosMin, PosMax);
        }
                         ...
                         TextureCacheType*      pTextureCache)
{
    ...
    if (!NewTextures.empty())
    {
        ...
        for (auto& Tex : NewTextures)
        {
            if (Tex)
            {
                StateTransitionDesc Barrier{Tex, RESOURCE_STATE_UNKNOWN, RESOURCE_STATE_SHADER_RESOURCE,STATE_TRANSITION_FLAG_UPDATE_STATE };
