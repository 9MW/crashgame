...
struct FileWindow :public IWidget
{
	struct Lablec
	{
		eastl::string mLabel;
	...
	eastl::vector<Lablec> fs;
	eastl::vector<PathHandle> subdir;
	Path* curentpath = NULL;
	int selectedid;
	PathHandle SelectedFile;
	FileSystem* fileSystem;
	void Updatelist() {
		subdir.clear();
		fsEnumerateFilesWithExtension(curentpath, " ", [](const Path* path, void* userData)
			{
				//if(path->cd)
	...
	FileWindow() :IWidget("files"), selectedid(-1) {
		fileSystem = fsGetSystemFileSystem();
		curentpath = fsCreatePath(fileSystem, "G:/");
		Updatelist();
		auto fdsp = subdir.size();

		for (size_t i = 0; i < subdir.size(); i++)
		{
			const Path* pth = subdir[i];
			Lablec lb = { fsGetPathDirectoryName(pth).buffer, fsDirectoryExists(pth) ? float4(0, 1, 0, 1) : float4(0xFFFF) };
			fs.push_back(lb);
		}
		auto stream = fsOpenFile(subdir[0], FileMode::FM_READ_WRITE);
		fsFreeFileSystem(fileSystem);
		/*	fsFreePath(directory);
			for (size_t i = 0; i < subdir.size(); i++)
			{
				 auto& pth = subdir[i];
				 pth.Release();
	...
	IWidget* Clone() const
	{
		FileWindow* pWidget = conf_placement_new<FileWindow>(conf_calloc(1, sizeof(*pWidget)));

		return pWidget;
	}
	void IWidget::Draw() {
		if (SelectedFile)
			ImGui::Text(fsGetPathAsNativeString(SelectedFile));
		for (size_t i = 0; i < fs.size(); i++)
		{
			/*if (ImGui::ColorButton(fs[i].mLabel.c_str(), fs[i].mColor)) {
				fs[i].mColor[0] = 1;
			}*/
			bool setd = i == selectedid;
			const char* sl = fs[i].mLabel.c_str();

			ImGui::PushID(i);
			if (fsDirectoryExists(subdir[i])) {
				auto p_min = ImGui::GetCursorScreenPos();
				auto p_max = float2(p_min.x + ImGui::GetContentRegionAvail().x, p_min.y + ImGui::GetFrameHeight());
				ImGui::GetWindowDrawList()->AddRectFilled(p_min, p_max, IM_COL32(0, 255, 0, 200));
			}
			if (ImGui::Selectable(sl, setd)) {

				selectedid = i;
				if (!fsDirectoryExists(subdir[i])) {
					SelectedFile.Release();
					SelectedFile = subdir[i];
				}
				else
				{
					selectedid = -1;
					/*PathComponent pcom;
					fsGetPathComponents(curentpath, &pcom, nullptr, nullptr);*/
					fsFreePath(curentpath);
					curentpath = fsCopyPath(subdir[i]);
					fs.clear();
					Updatelist();
					for (size_t i = 0; i < subdir.size(); i++)
					{
...
public:
	bool Init()
	{
		...
		gAppUI.mShowDemoUiWindow = false;

		{
			...
			CollapsingHeaderWidget CollapsingFileWidgets("Files");
			CollapsingFileWidgets.AddSubWidget(FileWindow());
			...
			pStandaloneControlsGUIWindow->AddWidget(CollapsingFileWidgets);
			pStandaloneControlsGUIWindow->AddWidget(SeparatorWidget());
	...
	void Draw()
	{
		...
		submitDesc.pEnqueueSignal = pRenderCompleteFence;
